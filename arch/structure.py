...
class ColorTransfer(nn.Module):
    def __init__(self):
        ...
        self.net1 = nn.Conv2d(4, 4, kernel_size=1, stride=1, padding=0, bias=False)
...
class FreTransfer(nn.Module):
    def __init__(self):
        ...
        self.net1 = nn.Conv2d(1, 2, kernel_size=(1, 2), stride=(1, 2), padding=0,
                              bias=False)  # Cin = 1, Cout = 4, kernel_size = (1,2)
...
class FreTransferInv(nn.Module):
    def __init__(self):
        ...
        self.net1 = nn.ConvTranspose2d(1, 1, kernel_size=(2, 1), stride=(2, 1), padding=0, bias=False)
        self.net1.weight = torch.nn.Parameter(g0_col)  # torch.Size([1,1,2,1])
        self.net2 = nn.ConvTranspose2d(1, 1, kernel_size=(2, 1), stride=(2, 1), padding=0, bias=False)
...
class MainDenoise(nn.Module):
    ...
    def forward(self, x, coeff_a=1, coeff_b=1):
        ...
        gamma, fusion_out, denoise_out, sigma = self.md1(ft0_d1, ft1_d1,
                                                         gamma_up_d2, denoise_out_d2,
                                                         coeff_a, coeff_b)
        denoise_up_d1 = self.fti(denoise_out)
        gamma_up_d1 = F.upsample(gamma, scale_factor=2)

        gamma, fusion_out, denoise_out, sigma = self.md0(ft0_d0, ft1_d0,
                                                         gamma_up_d1, denoise_up_d1, coeff_a, coeff_b)

        # refine
        refine_in = torch.cat([fusion_out, denoise_out, sigma], dim=1)  # 1 * 36 * 128 * 128
